# Intro to GitHub Classroom
This example project is written in Python, and tested with pytest.

### The assignment

The test in [`hello_test.py`](hello_test.py) is failing right now because the function in [`hello.py`](hello.py) isn't outputting the correct string. Fix this function and run the `pytest` command in the terminal to make the tests green. Next, commit and push your changes to GitHub using the "Source Control" button on the left sidebar. The autograder will run your tests and give you a grade, which you can check by clicking on the Github Actions extension on the left sidebar, hovering over the latest workflow run, and clicking the globe icon 🌐.

▶️ If you get stuck, refer to [the walkthrough video](https://github.com/AC-Classroom/intro-github-classroom/assets/45469701/93760bf7-0d27-49dc-8f66-7d50d428677f).


